...
import { CommentMdoel } from './comment.model';
import { sqlFragment } from './comment.provider';

/**
 * 创建评论
 */
export const createComment = async (comment: CommentMdoel) => {
  //准备查询
  const statement = `
        INSERT INTO comment
        SET ?
 ...
 * 修改评论
 */
export const updateComment = async (comment: CommentMdoel) => {
  //准备数据
...
export const getComments = async (options: GetCommentOptions) => {
  ...
  const statement = `
    SELECT
      ...
      ${sqlFragment.user},
      ${sqlFragment.post}
      ${filter.name == 'userReplied' ? `,${sqlFragment.replieComment}` : ''}
    FROM
      comment
